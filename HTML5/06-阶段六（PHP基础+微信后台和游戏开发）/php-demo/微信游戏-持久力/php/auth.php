<?php 
	...
	//http://ningbonuc.applinzi.com/wx_car/php/auth.php
	//1.引导用户点击这个链接，授权成功，存储用户信息，开始玩游戏
	//https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxb56f1725c4f779a9&redirect_uri=http://ningbonuc.applinzi.com/wx_car/php/auth.php&response_type=code&scope=snsapi_userinfo&state=canshu#wechat_redirect
	//code
	//print_r($_GET);
	//echo("<hr>");
	$code = $_GET["code"];
	...
	//echo $responseUser;
 	...
 	//保存到数据库
 	$sql = "INSERT INTO wx_car(openid,nickname,sex,headimgurl) VALUES('{$openid}','{$nickname}','{$sex}','{$headimgurl}')";
 	insertData($sql,function(){
 		global $nickname;
 		echo "<hr>正在加载".$nickname;
 	},function(){
 		global $nickname;
 	...
 	//http://ningbonuc.applinzi.com/wx_car/index.html

 	echo("<script>window.location.href='http://ningbonuc.applinzi.com/wx_car/index.html';</script>");
 ?>
